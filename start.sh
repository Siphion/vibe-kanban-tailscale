...
if pgrep -f "vibe-kanban" > /dev/null 2>&1; then
    ...
    echo "[*] Starting Vibe Kanban on 127.0.0.1:38100..."
    HOST=127.0.0.1 \
    PORT=38100 \
    VK_ALLOWED_ORIGINS="https://$TS_HOSTNAME" \
        nohup npx --yes vibe-kanban > "$LOGS_DIR/vibe-kanban.log" 2>&1 &
    VK_PID=$!
    sleep 2
    if kill -0 "$VK_PID" 2>/dev/null; then
        echo "[OK] Vibe Kanban started (PID $VK_PID)"
    else
        echo "[!] ERROR: Vibe Kanban failed to start. Check $LOGS_DIR/vibe-kanban.log"
        exit 1
    fi
fi
...
echo ""
echo "=== Running ==="
echo "Vibe Kanban      : http://127.0.0.1:38100 (local only)"
